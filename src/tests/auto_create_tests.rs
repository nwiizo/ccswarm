...
async fn test_auto_create_engine_initialization() {
    ...
    let tasks = engine.analyze_and_decompose("Create a TODO app").await.unwrap();
...
async fn test_app_type_detection() {
    ...
    // Test TODO detection
    let tasks = engine.analyze_and_decompose("Create a TODO application").await.unwrap();
    assert!(!tasks.is_empty());
    
    // Test Blog detection
    let tasks = engine.analyze_and_decompose("Create a blog website").await.unwrap();
    assert!(!tasks.is_empty());
    
    // Test multiple app types are detected properly
    let tasks = engine.analyze_and_decompose("Create an online shop").await.unwrap();
    assert!(!tasks.is_empty());
}
...
async fn test_task_decomposition() {
    ...
    let tasks = engine.analyze_and_decompose("Create a TODO app with authentication").await.unwrap();
    ...
    assert!(tasks.iter().any(|t| t.description.contains("authentication") || t.description.contains("JWT")));
}
...
async fn test_responsive_design_customization() {
    ...
    let tasks = engine.analyze_and_decompose("Create a mobile-responsive TODO app").await.unwrap();
    
    // Should have mobile/responsive mentioned in frontend task
    let frontend_task = tasks.iter()
...
async fn test_realtime_features_customization() {
    ...
    let tasks = engine.analyze_and_decompose("Create a real-time chat application").await.unwrap();
    
    // Should have WebSocket task
    assert!(tasks.iter().any(|t| 
        t.description.contains("real-time") || 
        t.description.contains("WebSocket")
    ));
}

fn create_test_config() -> CcswarmConfig {
    use std::collections::HashMap;
    use crate::config::{AgentConfig, ClaudeConfig, ProjectConfig, RepositoryConfig, MasterClaudeConfig, CoordinationConfig, ThinkMode};
