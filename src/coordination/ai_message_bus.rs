...
impl AICoordinationBridge {
    ...
    pub async fn register_agent(
        ...
        self.message_bus.publish_to_agent(&dummy_agent, registration).await.ok(); // Ignore error
    ...
    pub async fn delegate_task(
        ...
        let ai_agent_id = agent_map.get(agent_id)
        ...
        self.message_bus.publish_to_agent(&ai_agent_id, assignment).await?;
    ...
    async fn handle_agent_message(
        ...
        match msg {
            ...
            AgentMessage::TaskProgress { agent_id, task_id, progress, message } => {
            ...
            AgentMessage::StatusUpdate { agent_id, status, metrics } => {
    ...
    async fn handle_task_completion(
        ...
        // Parse task result
        let task_result: TaskResult = serde_json::from_value(result_data)
    ...
    async fn handle_help_request(
        ...
        tracing::info!("Help request from agent {} with priority {}", agent_id, priority);
        
        // Find the ccswarm agent ID
        let agent_map_guard = agent_map.read().await;
        let ccswarm_agent_id = agent_map_guard.iter()
        ...
        if let Some(ccswarm_id) = ccswarm_agent_id {
            ...
            message_bus.publish_to_agent(&agent_id, help_response).await?;
        }
    ...
    pub async fn send_to_agent(&self, agent_id: &str, message: AgentMessage) -> Result<()> {
        ...
        self.message_bus.publish_to_agent(ai_agent_id, message).await?;
        Ok(())
    }
...
trait MasterClaudeCoordination {
    ...
    async fn handle_help_request(&self, agent_id: &str, context: serde_json::Value) -> Result<serde_json::Value>;
}
