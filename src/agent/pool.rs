...
use crate::agent::{ClaudeCodeAgent, Task, TaskResult, AgentStatus};
...
use crate::coordination::{CoordinationBus, AgentMessage};
...
impl AgentPool {
    ...
    pub async fn spawn_agent(&mut self, agent_type: &str, config: &CcswarmConfig) -> Result<()> {
        ...
        // Get agent config
        let agent_config = config.agents.get(agent_type)
    ...
    pub async fn get_best_agent_for_task(&self, task: &Task) -> Result<Arc<RwLock<ClaudeCodeAgent>>> {
    ...
    pub async fn execute_task_with_agent(
        ...
        info!("📋 Executing task with {} agent: {}", agent_type, task.description);
        ...
        // Update agent status
        {
            ...
            // Add to task history
            agent_guard.task_history.push((task.clone(), result.clone()));
        }
        
        // Send completion message
        self.coordination_bus.send_message(AgentMessage::TaskCompleted {
            agent_id: agent_type.to_string(),
            task_id: task.id.clone(),
            result: result.clone(),
        }).await?;
