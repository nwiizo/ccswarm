...
| Faceted prompting | Partial — builtin + project layers; no `~/.ccswarm/facets` user layer. 6 personas / 4 policies vs external-workflow's 26 / richer set |
| Providers | claude (deep) / codex / copilot (intentional no-op); external-workflow adds cursor, kiro, opencode, claude-sdk |
...
1. **Orchestrator-worker escape hatch** — the one industry-standard
   ...
   subtasks at runtime (external-workflow's team_leader). Most strategically
   significant gap.
2. **Rate-limit fallback chain** — detect 429s in the bridge and retry
   on the next provider in a configured chain (external-workflow v0.41.0).
3. **Step-level provider promotion** — escalate model/provider after N
   executions of the same stage (external-workflow v0.41.0).
4. **Command quality gates** — machine-executed gates (build/lint) whose
   failure feeds back into the same stage (external-workflow v0.43.0).
