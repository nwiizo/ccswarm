...
This section re‑introduces four CLI surfaces with external-workflow‑aligned patterns. They are designed as thin handlers that delegate business logic to coordination modules, store state as JSON files, and support dual output (text/JSON).

### external-workflow Patterns Applied
