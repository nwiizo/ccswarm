...
use crate::{SessionConfig, SessionManager as InnerSessionManager, AISession};
use crate::core::{SessionId, SessionStatus};
use crate::persistence::{PersistenceManager, SessionState, SessionMetadata};
use anyhow::Result;
use std::path::PathBuf;
use std::sync::Arc;

/// Global session manager instance
static MANAGER_INSTANCE: tokio::sync::OnceCell<Arc<PersistentSessionManager>> = tokio::sync::OnceCell::const_new();

/// Get the global persistent session manager
pub async fn get_session_manager() -> Result<Arc<PersistentSessionManager>> {
    Ok(MANAGER_INSTANCE
        .get_or_init(|| async {
            Arc::new(PersistentSessionManager::new().await.unwrap())
        })
        .await
        .clone())
}
...
impl PersistentSessionManager {
                ...
                Ok(state) => {
                    ...
                    match state.status {
                        SessionStatus::Running | SessionStatus::Paused => {
                            ...
                            match self.inner.restore_session(
    ...
    pub async fn create_session_with_config(&self, config: SessionConfig) -> Result<Arc<AISession>> {
        ...
        eprintln!("Saving session {} with status {:?}", session.id, state.status);
        ...
        if let Some(session) = self.inner.get_session(id) {
            ...
            match self.persistence.load_session(id).await {
                Ok(state) => {
                    ...
                    match self.inner.create_session_with_config(state.config.clone()).await {
                        Ok(session) => {
                            // Start the session
                            if let Err(e) = session.start().await {
