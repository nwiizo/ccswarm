...
async fn main() -> Result<()> {
    ...
    let mut search_agent = SearchAgent::new(
        "search-agent-demo".to_string(),
    ...
    let member = SanghaMember {
        ...
        role: AgentRole::Search {
            ...
            responsibilities: vec!["Proposal Research".to_string(), "Evidence Gathering".to_string()],
    ...
    for proposal in &proposals {
        ...
        // 3. Analyze results
        let positive_indicators = all_results.iter()
            .filter(|r| r.snippet.contains("effective") || 
