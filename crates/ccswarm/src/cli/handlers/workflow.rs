...
impl CliRunner {
    ...
    pub(crate) async fn handle_pipeline(
        ...
        // Post-execution: auto-commit and PR creation
        if auto_commit && result.is_success() {
            eprintln!("{}", "Auto-committing changes...".bright_cyan());
            let commit_output = tokio::process::Command::new("git")
                .args(["add", "-A"])
                .current_dir(&self.repo_path)
                .output()
                .await;
            if commit_output.is_ok() {
                ...
                let _ = tokio::process::Command::new("git")
                    ...
                    .output()
                    .await;
                eprintln!("  {} Changes committed", "\u{2713}".bright_green());
            }
        }

        if create_pr && result.is_success() {
            eprintln!("{}", "Creating pull request...".bright_cyan());
            ...
            match pr_output {
                Ok(output) if output.status.success() => {
                    ...
                    eprintln!("  {} PR created: {}", "\u{2713}".bright_green(), url.trim());
                }
                _ => {
                    eprintln!("  {} PR creation failed (is gh cli installed?)", "\u{2717}".bright_red());
                }
            }
        }
