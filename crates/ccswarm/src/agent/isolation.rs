...
mod tests {
    ...
    fn test_isolation_mode_default() {
        assert_eq!(IsolationMode::default(), IsolationMode::GitWorktree);
    }

    #[test]
    fn test_isolation_mode_requires_docker() {
        assert!(!IsolationMode::GitWorktree.requires_docker());
        assert!(IsolationMode::Container.requires_docker());
        assert!(IsolationMode::Hybrid.requires_docker());
    }

    #[test]
    fn test_isolation_mode_uses_worktree() {
        assert!(IsolationMode::GitWorktree.uses_worktree());
        assert!(!IsolationMode::Container.uses_worktree());
        assert!(IsolationMode::Hybrid.uses_worktree());
    }

    #[test]
    fn test_isolation_mode_from_str() {
        assert_eq!(
            IsolationMode::from_str("worktree"),
            Some(IsolationMode::GitWorktree)
        );
        assert_eq!(
            IsolationMode::from_str("container"),
            Some(IsolationMode::Container)
        );
        assert_eq!(
            IsolationMode::from_str("docker"),
            Some(IsolationMode::Container)
        );
        assert_eq!(
            IsolationMode::from_str("hybrid"),
            Some(IsolationMode::Hybrid)
        );
        assert_eq!(IsolationMode::from_str("invalid"), None);
    }

    #[test]
    fn test_isolation_config_default() {
        let config = IsolationConfig::default();
        assert_eq!(config.mode, IsolationMode::GitWorktree);
        assert!(!config.enforce);
        assert!(config.container.auto_remove);
    }
}
