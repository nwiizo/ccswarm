...
impl SubagentSessionManager {
    ...
    pub async fn create_session(
        ...
        let _agent_session = adapter
            ...
            .await
            .map_err(|e| {
                SubagentError::Delegation(format!("Failed to create agent session: {}", e))
            })?;
        ...
        log::info!(
            "Created AI-Session for subagent {}: {}",
            instance_id,
            session_id
        );
    ...
    pub async fn execute_command(&self, session_id: &str, command: &str) -> SubagentResult<String> {
        let mut sessions = self.sessions.write().await;
    ...
    pub async fn send_input(&self, session_id: &str, input: &str) -> SubagentResult<()> {
        let mut sessions = self.sessions.write().await;
    ...
    pub async fn get_output(&self, session_id: &str) -> SubagentResult<String> {
    ...
    pub async fn get_ai_context(&self, session_id: &str) -> SubagentResult<String> {
        let sessions = self.sessions.read().await;
        ...
        Ok(format!(
            "Session context: {} messages, agent state: {:?}",
    ...
    pub async fn compress_context(&self, session_id: &str) -> SubagentResult<()> {
        let mut sessions = self.sessions.write().await;
        ...
        session.metadata.tokens_saved = stats.estimated_token_savings;
        log::info!(
            "Session {} efficiency: saved {} tokens",
            session_id,
            stats.estimated_token_savings
        );
    ...
    pub async fn send_coordination_message(
        ...
        let from = sessions.get(from_session).ok_or_else(|| {
            SubagentError::NotFound(format!("From session not found: {}", from_session))
        })?;

        let to = sessions.get(to_session).ok_or_else(|| {
            SubagentError::NotFound(format!("To session not found: {}", to_session))
        })?;
    ...
    pub async fn get_session_stats(&self, session_id: &str) -> SubagentResult<SessionMetadata> {
        let sessions = self.sessions.read().await;
    ...
    /// Clean up a session
    pub async fn cleanup_session(&self, session_id: &str) -> SubagentResult<()> {
        let mut sessions = self.sessions.write().await;
        ...
        if let Some(session) = sessions.remove(session_id) {
            ...
            session
                .adapter
                .terminate_session(&session.instance_id)
                .await
                .map_err(|e| {
                    SubagentError::Delegation(format!("Failed to cleanup session: {}", e))
                })?;
...
mod tests {
    ...
    fn test_subagent_session_config_default() {
        ...
        assert_eq!(
            ...
            Some(".ccswarm/sessions".to_string())
        );
    }
}
