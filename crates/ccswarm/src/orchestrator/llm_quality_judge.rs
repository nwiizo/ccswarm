...
mod tests {
    ...
    fn test_quality_rubric_defaults() {
        ...
        assert_eq!(rubric.dimensions.get("correctness").unwrap(), &0.3);
        assert_eq!(rubric.thresholds.get("test_quality").unwrap(), &0.85);
    }
    ...
    fn test_role_specific_weights() {
        ...
        assert_eq!(backend_weights.get("security").unwrap(), &0.2);
    }
    ...
    #[tokio::test]
    async fn test_heuristic_evaluation() {
        let mut judge = LLMQualityJudge::new();
        judge.use_claude = false; // Force heuristics
        ...
        assert!(!evaluation.issues.is_empty()); // Should find missing tests
    }
}
