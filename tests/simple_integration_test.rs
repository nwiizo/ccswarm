...
use ccswarm::coordination::{
    dialogue::{DialogueCoordinationBus, ConversationType, ConversationContext, DialogueMessageType, EmotionalTone, ResponseExpectation},
    CoordinationBus, AgentMessage,
};
...
async fn test_session_memory_integration() -> Result<()> {
    ...
    // Test task context
    memory.set_task_context("task-123".to_string(), "User authentication implementation".to_string());
    ...
    memory.add_episode(
        ...
        context,
        EpisodeOutcome::Success { metrics: HashMap::new() },
    );
    ...
    assert!(!retrieval_result.working_memory_items.is_empty() || 
...
async fn test_dialogue_coordination_integration() -> Result<()> {
    ...
    let context = ConversationContext {
        ...
        goals: vec!["implement secure auth".to_string(), "maintain UX quality".to_string()],
        related_conversations: vec![],
    };
    
    let conversation_id = dialogue_bus.start_conversation(
        participants.clone(),
    ...
    dialogue_bus.add_dialogue_message(
        &conversation_id,
    ...
    dialogue_bus.add_dialogue_message(
        ...
        "I'll provide POST /auth/login and POST /auth/register endpoints with JWT response".to_string(),
    ...
    let summary = dialogue_bus.get_conversation_summary(&conversation_id).unwrap();
...
async fn test_comprehensive_feature_validation() -> Result<()> {
    ...
    let mut personality = AgentPersonality::new("comprehensive-test".to_string(), &default_frontend_role());
    ...
    let mut memory = SessionMemory::new("comprehensive-session".to_string(), "comprehensive-agent".to_string());
    
    // Working memory
    memory.add_to_working_memory("Critical task data".to_string(), WorkingMemoryType::TaskInstructions, 0.9);
    ...
    session.add_memory("Integration test data".to_string(), WorkingMemoryType::IntermediateResult, 0.8);
    session.set_task_context("integration-task".to_string(), "Validate all integrations".to_string());
    
    // Episode with detailed context
    let mut episode_context = HashMap::new();
    episode_context.insert("test_type".to_string(), "integration".to_string());
    episode_context.insert("complexity".to_string(), "high".to_string());
    episode_context.insert("success_factors".to_string(), "coordination,memory,dialogue".to_string());
